package com.sulake.core.window.utils
{
   import com.sulake.core.window.enum.WindowParam;
   import flash.utils.Dictionary;
   
   public class ParamCodeTable extends WindowParam
   {
       
      
      public function ParamCodeTable()
      {
         super();
      }
      
      public static function fillTables(param1:Dictionary, param2:Dictionary = null) : void
      {
         var _loc3_:* = null;
         param1["null"] = const_114;
         param1["bound_to_parent_rect"] = const_79;
         param1["child_window"] = const_886;
         param1["embedded_controller"] = const_297;
         param1["resize_to_accommodate_children"] = const_61;
         param1["input_event_processor"] = const_48;
         param1["internal_event_handling"] = const_655;
         param1["mouse_dragging_target"] = const_191;
         param1["mouse_dragging_trigger"] = const_281;
         param1["mouse_scaling_target"] = const_258;
         param1["mouse_scaling_trigger"] = const_432;
         param1["horizontal_mouse_scaling_trigger"] = const_215;
         param1["vertical_mouse_scaling_trigger"] = const_216;
         param1["observe_parent_input_events"] = const_914;
         param1["optimize_region_to_layout_size"] = const_353;
         param1["parent_window"] = const_839;
         param1["relative_horizontal_scale_center"] = const_159;
         param1["relative_horizontal_scale_fixed"] = const_107;
         param1["relative_horizontal_scale_move"] = const_324;
         param1["relative_horizontal_scale_strech"] = const_245;
         param1["relative_scale_center"] = const_788;
         param1["relative_scale_fixed"] = const_568;
         param1["relative_scale_move"] = const_838;
         param1["relative_scale_strech"] = const_961;
         param1["relative_vertical_scale_center"] = const_167;
         param1["relative_vertical_scale_fixed"] = const_110;
         param1["relative_vertical_scale_move"] = const_334;
         param1["relative_vertical_scale_strech"] = const_231;
         param1["on_resize_align_left"] = const_658;
         param1["on_resize_align_right"] = const_422;
         param1["on_resize_align_center"] = const_418;
         param1["on_resize_align_top"] = const_567;
         param1["on_resize_align_bottom"] = const_402;
         param1["on_resize_align_middle"] = const_480;
         param1["on_accommodate_align_left"] = const_855;
         param1["on_accommodate_align_right"] = const_414;
         param1["on_accommodate_align_center"] = const_621;
         param1["on_accommodate_align_top"] = const_1019;
         param1["on_accommodate_align_bottom"] = WINDOW_PARAM_ON_ACCOMMODATE_ALIGN_BOTTOM;
         param1["on_accommodate_align_middle"] = const_535;
         param1["route_input_events_to_parent"] = const_386;
         param1["use_parent_graphic_context"] = const_31;
         param1["draggable_with_mouse"] = const_999;
         param1["scalable_with_mouse"] = const_806;
         param1["reflect_horizontal_resize_to_parent"] = const_468;
         param1["reflect_vertical_resize_to_parent"] = WINDOW_PARAM_REFLECT_VERTICAL_RESIZE_TO_PARENT;
         param1["reflect_resize_to_parent"] = const_229;
         if(param2 != null)
         {
            for(_loc3_ in param1)
            {
               if(param2[param1[_loc3_]] != null)
               {
                  if(param1[_loc3_] != 0)
                  {
                     Logger.log("Conflictiong flags in window params: \"" + _loc3_ + "\"!");
                  }
               }
               param2[param1[_loc3_]] = _loc3_;
            }
         }
      }
   }
}
